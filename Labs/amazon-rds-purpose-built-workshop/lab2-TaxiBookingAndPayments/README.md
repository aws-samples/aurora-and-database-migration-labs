...
1. Open the AWS Management Console for [AWS Cloud9](https://us-east-1.console.aws.amazon.com/cloud9/home/account). You will leverage AWS Cloud9 IDE throughout this lab for running scripts, deploying AWS SAM (Serverless Application Model) templates, execute SQL queries etc.
...
4. Under __Stacks__ copy and make a note of the name of the parent Amazon CloudFormation stack starting with **mod-** (e.g. _mod-aa8afde9acf04c7f_).
5. Substitute the string (_substitute-name-of-copied-cf-stack-name_) in the command below with the name of the Amazon CloudFormation stack and run it in the AWS Cloud9 terminal window.
...
> Note: Ensure that the name of the Amazon CloudFormation stack printed in the terminal window (e.g. _mod-aa8afde9acf04c7f_) matches with the name of your parent Amazon CloudFormation stack.
...
In this section, you will be using AWS Serverless Application Model ([SAM]((https://aws.amazon.com/serverless/sam/))) CLI to deploy a Lambda Function within the same Amazon Virtual Private Network([VPC](https://aws.amazon.com/vpc/)). The SAM deployment will also include a python interface to the PostgreSQL database engine as an AWS Lambda Layer. This Lambda function will read the taxi trip information from the Amazon DynamoDB stream as they are inserted/updated in the Amazon DynamoDB table **_aws-db-workshop-trips_**. Only when a trip is complete (denoted by the _STATUS_ attribute in the trip item/record) the Lambda function will insert information into a relational table **_trips_** in Amazon Aurora PostgreSQL database. 
...
> Note: You can copy the entire script block below and paste in the terminal window. Press Enter for the final command to execute.
...
6. Copy and paste the following command to deploy the AWS Lambda Function along with the Lambda Layer.
...
```
>Note: This will take a few minutes. Ensure that the SAM teamplate was successfully deployed. Look for the following line in the terminal as output
>```
...
> From the output of the script make a note of the tripinfo value. You will be entering this value when prompted by the subsequent scripts. It a randomly generated string that uniquely identifies a trip. It will be similar to 
>```
> "tripinfo": "2020-04-05T18:52:42Z,8987397"
>``` 

2. Copy and paste the following command as a driver to accept a trip. The script will prompt for the 'tripinfo' value. Enter the value (without double quotes for e.g. 2020-04-05T18:52:42Z,8987397) from the output of the previous python script you just ran as a rider to book a new trip.
...
>Note: The trip status is **_COMPLETE_** now and so the AWS Lambda function would have picked the record from DynamoDB and inserted into Aurora PostgreSQL. 
...
5. Execute the query below to insert driver payment information for all the drivers, for the current billing cycle based on the billing information in the billing table.
