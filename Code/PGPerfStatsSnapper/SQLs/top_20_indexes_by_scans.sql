with pg_stat_all_indexes_vw as  (
 ...
 idx_scan       ,
 idx_tup_read   ,
 idx_tup_fetch  from pg_stat_all_indexes_history a, pg_awr_snapshots_cust b where a.snap_id = b.snap_id and a.snap_id between :begin_snap_id and :end_snap_id),
 ...
 select indexrelid,schemaname,relname,indexrelname,sum(delta_idx_scan) AS total_index_scan,sum(delta_idx_tup_read) AS Total_index_tuple_reads,sum(delta_idx_tup_fetch) AS Total_index_tuple_fetch
 from get_delta_data
 group by indexrelid,schemaname,relname,indexrelname
 having (sum(delta_idx_scan) is not null )
 order by 5 DESC
limit 20;
